import { ArrowRight, Blocks, Building2, ChevronDown, ChevronLeft, ChevronUp, Download, File, Info, Plus, Shapes, SlidersHorizontal, Trash, User, Users, X } from "lucide-react";
                     ...
                     <div>
                      ...
                      >
                       ...
                       const avatar = avatarMap[id] || "";
                            return (
                              <div className="flex w-full cursor-pointer" onClick={() => handlePesquisaFinal(item.id)}>
                              
                                <div
        
                                  className={cn(
                                    "flex flex-col rounded-lg w-full  bg-white dark:bg-neutral-800 dark:border-neutral-700 items-start gap-2  border p-3 text-left text-sm transition-all hover:bg-accent",
                          ...
                          {item.name}
                                    ...
                                    {!version && (
                                          <div className="text-gray-500 text-sm flex gap-1 items-center"><Building2 size={12} />{item.institution}</div>
                                    )}

<div className="text-gray-500 text-sm flex gap-1 items-center">  <div className={`w-3 min-w-3 h-3 rounded-sm  ${qualisColor[normalizeArea(item.area || '')]} relative`}></div>
{item.area}</div>
                                    ...
                                    <p className="text-sm text-gray-500">Líderes:</p>
          <div className="flex items-center ">
                   ...
                   <Avatar 
                     ...
                     onClick={(event) => {
                       ...
                       onOpen('researcher-modal', { name: item.first_leader });
                     }} 
                   ...
                   >
                     <AvatarImage 
                       ...
                       src={`${urlGeral}ResearcherData/Image?name=${item.first_leader}`} 
                     />
                     <AvatarFallback className="flex items-center justify-center">
                       <User size={16} />
                     </AvatarFallback>
                   ...
                   {(item.second_leader != '' && item.second_leader != null) && (
 <Avatar 
                   
 onClick={(event) => {
   event.stopPropagation(); 
   onOpen('researcher-modal', { name: item.second_leader });
 }} 
 className="cursor-pointer rounded-full relative border dark:border-neutral-800 h-8 w-8 hover:z-10 transition-transform hover:scale-110"
 style={{
   marginLeft:  '-10px'
...
>
 <AvatarImage 
   className="rounded-md h-8 w-8" 
   src={`${urlGeral}ResearcherData/Image?name=${item.second_leader}`} 
 />
 <AvatarFallback className="flex items-center justify-center">
   <User size={16} />
 </AvatarFallback>
