import { ArrowLeftFromLine, ArrowRightFromLine, BarChartBig, Blocks, Book, BookOpen, Briefcase, Building2, Calendar, ChevronDown, ChevronLeft, ChevronUp, Copyright, File, Globe, GraduationCap, Hash, Home, Info, LayoutDashboard, LoaderCircle, MapPinIcon, SlidersHorizontal, SquareLibrary, Star, TextSearch, Ticket, Undo2, User, Users, Users2, X } from "lucide-react";
...
import { DocentesPrograma } from "../graduate-program/docentes-programa";
import { IndicatorsGraduate } from "../graduate-program/indicators-graduate";
...
import { GraficoIndiceProdBibli } from "../graduate-program/grafico-indice-producao-bibliografica";
import { useTheme } from "next-themes";
import { LogoConecteeWhite } from "../svg/LogoConecteeWhite";
import { LogoConectee } from "../svg/LogoConectee";

import { Badge } from "../ui/badge";
import { HomepageProgram } from "../graduate-program/homepage-program";
import { PainelAdminGraduate } from "../graduate-program/painel-admin-graduate";
import { Helmet } from "react-helmet";
import { Avatar, AvatarFallback, AvatarImage } from "../ui/avatar";
import { ProducoesPrograma } from "../graduate-program/producoes-programa";
import { LinhasPesquisaPrograma } from "../graduate-program/linhas-pesquisa-programa";
...
import { IndicatorsDepartament } from "./indicators-departamento";
import { HomepageDepartament } from "./homepage-departament";
...
export function VisualizacaoDepartamento() {
  ...
  const [graduatePrograms, setGraduatePrograms] = useState<Departament>();

  const urlGraduateProgram = `${urlGeral}ufmg/departamentos?dep_id=${type_search}`;
  ...
  const siteTitle = graduatePrograms?.dep_nom
  ? `${graduatePrograms.dep_nom} | ${version ? "Conectee" : "Simcc"}`
    ...
    >
      ...
      <p className="text-9xl text-[#719CB8] font-bold mb-16 animate-pulse">
        ...
        <h1 className="text-center text-2xl md:text-4xl text-neutral-400 font-medium leading-tight tracking-tighter lg:leading-[1.1] ">
          Não foi possível acessar as <br/>  informações deste departamento.
        </h1>
...
<div    style={{ backgroundImage: `url(${keepoData.app.background_image})` }} className="bg-eng-blue bg-no-repeat bg-center bg-cover border dark:border-neutral-800 w-full rounded-md h-[300px]">
 <div className={`w-full h-full relative rounded-md ${!(keepoData.app.background_image == "") && ('bg-black/25 ')}  pb-0 md:pb-0 p-4 md:p-8 flex-col flex justify-between `}>
  <div
    ...
    <Link to={`/dashboard/departamento?dep_id=${type_search}`} target="_blank">
 ...
 <div className="absolute">
  {keepoData.profile_info.avatar ? (
    ...
    <AvatarFallback className="flex items-center justify-center"><Building2 size={24} /></AvatarFallback>
  </Avatar>
  )}
...
<HomepageDepartament program={graduatePrograms} keepoData={keepoData}/>
</TabsContent>
...
<DocentesDepartamento departament={graduatePrograms} />
</TabsContent>


<TabsContent value="indicadores" className="m-0">
<IndicatorsDepartament />
</TabsContent>
