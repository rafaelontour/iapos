...
import { AdminGraficoGraduateProgram } from "../graduate-program/admin-graficos-graduate-program";
...
import { Departament } from "./visualizacao-departamento";
...
export function HomepageDepartament(props: Props) {
  ...
  const urlTermPublicacoes = urlGeral + `recently_updated?year=${new Date().getFullYear() - 4}&university=&dep_id=${type_search}`
...
console.log('(urlDados',urlDados)
  ...
  let urlPalavrasChaves = `${urlGeral}lists_word_researcher?dep_id=${type_search}&researcher_id=`
...
//////////////////////////////// KEEPO PAGE
  ...
  return (
    <main className="h-full w-full flex flex-col px-4 md:px-8 pb-4 md:pb-8">
      {(props.program.dep_email || props.program.dep_site || props.program.dep_tel) && (
           <Alert className="mb-8">
           <div className="flex flex-wrap gap-3">
             {props.program.dep_email && (
                               <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center"><Mail size={12} />{props.program.dep_email}</div>
                             )}

{props.program.dep_site && (
                               <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center"><Globe size={12} />{props.program.dep_site}</div>
                             )}

{props.program.dep_tel && (
                               <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center"><Phone size={12} />{props.program.dep_tel}</div>
                             )}
           </div>
           </Alert>
      )}

      {props.program.dep_des && (
         <Alert className="mb-8">
          <h3 className="text-2xl font-medium mb-2">Descrição</h3>
