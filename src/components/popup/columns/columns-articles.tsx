    ...
    cell: ({ row }) => {
      ...
      return <div>
        {(jif != "None" && jif.length != 0) && (
          <a href={row.getValue("jcr_link")} target="_blank" rel="noopener noreferrer" className=" cursor-pointer flex  rounded-md text-xs font-medium gap-2 items-center  whitespace-nowrap"><LinkBreak size={16} className="" />JCR {jif}</a>
        )}
      </div>
    }
  },
