...
import { GraficoTotalSoftware } from "./graficos/grafico-software-ano"
          ...
          const updateDistinct = useCallback(
            ...
            []
                ...
                const totalAmong = total.reduce((acc, item) => acc + item.among, 0);
                    ...
                    let urlTotais = `${urlGeral}software_metrics?type=BOOK&term=${valoresSelecionadosExport}&year=${yearString || 1990}&distinct=${distinct ? '1' : '0'}&area=${arrayToParam(selectedAreas)}&graduate_program=${arrayToParam(selectedGraduatePrograms)}&city=${arrayToParam(selectedCities)}&institution=${arrayToParam(selectedUniversities)}&modality=${arrayToParam(selectedSubsidies)}&graduation=${arrayToParam(selectedGraduations)}&departament=${arrayToParam(selectedDepartaments)}&graduate_program_id=${graduate_program_id ? graduate_program_id : ''}&dep_id=${dep_id ? dep_id : ''}`;
...
<Accordion  type="single" collapsible defaultValue="item-1">
                ...
                <HeaderResultTypeHome title="Gráfico de quantidade total de softwares" icon={<ChartBar size={24} className="text-gray-400" />}>
                    ...
                    {loading ? (
                      ...
                      <GraficoTotalSoftware total={total} />
