...
import { GraficoTotalTextoRevista } from "./graficos/grafico-texto-revista-ano"
          ...
          const updateDistinct = useCallback(
            ...
            []
                ...
                const totalAmong = total.reduce((acc, item) => acc + item.among, 0);
                    ...
                    let urlTotais = `${urlGeral}papers_magazine_metrics?type=BOOK&term=${valoresSelecionadosExport}&year=${yearString || 1990}&distinct=${distinct ? '1' : '0'}&area=${arrayToParam(selectedAreas)}&graduate_program=${arrayToParam(selectedGraduatePrograms)}&city=${arrayToParam(selectedCities)}&institution=${arrayToParam(selectedUniversities)}&modality=${arrayToParam(selectedSubsidies)}&graduation=${arrayToParam(selectedGraduations)}&departament=${arrayToParam(selectedDepartaments)}&graduate_program_id=${graduate_program_id ? graduate_program_id : ''}&dep_id=${dep_id ? dep_id : ''}`;
             ...
             <Alert className={`p-0 bg-cover bg-no-repeat bg-center `}  >
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">
                      Total de texto em revista ou jornal
                    </CardTitle>
...
<Accordion  type="single" collapsible defaultValue="item-1">
                ...
                <HeaderResultTypeHome title="Gráfico de quantidade total de marcas" icon={<ChartBar size={24} className="text-gray-400" />}>
                    ...
                    {loading ? (
                      ...
                      <GraficoTotalTextoRevista total={total} />
            ...
            <p className=" font-medium"> Texto em revista ou jornais</p>
            </div>
