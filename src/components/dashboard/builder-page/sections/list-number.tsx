...
export function ListNumberSection (props:Props) {
  ...
  const addSocialItem = (index: number) => {
    props.setKeepoData((prev) => {
      ...
      // Verifica se o item na posição especificada é uma lista numerada
      if (updatedContent[index] && updatedContent[index].type === "list-number") {
        // Se já for uma lista numerada, adiciona um novo item à lista
        updatedContent[index].items.push({ name: '', url: '', title: "", image: "" });
      } else {
        // Caso não seja uma lista numerada, cria um novo item de lista e o insere na posição especificada
